...
function run_httpclnt()
    ...
    resp = HTTP.get("http://localhost:$(SRVR_PORT)/"; status_exception=false)
    @test resp.status == 404

    resp = HTTP.get("http://localhost:$(SRVR_PORT)/invalidapi"; status_exception=false)
    @test resp.status == 404

    respstr = String(HTTP.get("http://localhost:$(SRVR_PORT)/testfn1/1/2"; status_exception=false).body)
    resp = JSON.parse(respstr)
    @test resp["code"] == 0
    @test resp["data"] == 5

    respstr = String(HTTP.get("http://localhost:$(SRVR_PORT)/testfn1/1/2"; query=Dict(:narg1 => 3, :narg2 => 4), status_exception=false).body)
    resp = JSON.parse(respstr)
    @test resp["code"] == 0
    @test resp["data"] == 11
    ...
    respstr = String(HTTP.post("http://localhost:$(SRVR_PORT)/testFile",
        headers=headers, body=postdata; status_exception=false).body)
    resp = JSON.parse(respstr)
    @test resp["code"] == 0
    @test resp["data"] == "5,446"

    println("testing preprocessor...")
    resp = HTTP.get("http://localhost:$(SRVR_PORT)/testfn1/1/2"; headers=Dict("juliawebapi" => "404"), status_exception=false)
    @test resp.status == 404
